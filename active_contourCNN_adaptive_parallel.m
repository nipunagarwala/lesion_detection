function [phi,Rad,Data1, Data2,ParamsVec] =active_contourCNN_adaptive_parallel(im,phi,ObjectSize,mu,nu, lambda1, lambda2,numIter,NarrowBandSize, ...
    ...
    % myRads is an array for window size per pixel in contour
     if (ii == 1) 
        myRads(1:length(idx)) = 5;    %local window X
         ...
         tmpdirforpool = '/scratch/users/nipuna1/matlab_pool_folder/';
         mkdir(tmpdirforpool)
         cluster.JobStorageLocation = tmpdirforpool
         msg = sprintf(['setting matlabpool to'  num2str(number_slots)])
         cluster.NumWorkers = number_slots
         parpool(cluster)
        
        i = 1;
        c = 1;
         while i<=length(i_array)
             step = round(length(i_array)/number_slots);
